"""Admin in-app notification endpoints — Phase 5 contract §6."""
...
_RECIPIENT = NotificationRecipientType.ADMIN


@router.get("", summary="List admin notifications")
async def list_notifications(
    session: AsyncSession = Depends(get_db),
    admin: AdminUser = Depends(require_admin_permission(Permission.ADMIN_NOTIFICATION_READ)),
...
async def unread_count(
    ...
    n = await notification_service.get_unread_count(
        session, recipient_type=_RECIPIENT, recipient_id=admin.id
    )
...
async def delete_notification(
    ...
    await notification_service.delete_(
        session, notification_id, recipient_type=_RECIPIENT, recipient_id=admin.id
    )
